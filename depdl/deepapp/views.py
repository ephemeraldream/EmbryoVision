...
def upload_image(request):
    ...
    if request.method == 'POST':
        ...
        if image_file:
            ...
            return render(request, 'C:\\Work\\EmbryoVision\\EmbryoVision\\depdl\\deepapp\\templates\\image_work.html', {'image_url': image_url, "image_id": image_object.id})
...
def put_labels(request):
    if request.method == 'POST':
        ...
        gas_result = 'Gas outside of the holes detected and probably covers some part of the image.' \
            if hole_pred == 1 else 'Gas outside of the holes is not Detected. The image is clean.'


        return JsonResponse({'message': "Labels put on the image",
